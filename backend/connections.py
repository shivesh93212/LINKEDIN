...
def pending_request(current_user=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    return db.query(Connection).options(joinedload(Connection.sender),joinedload(Connection.receiver)).filter(Connection.receiver_id==current_user.id,Connection.status=="pending").all()
